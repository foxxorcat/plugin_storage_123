...
func (d *Pan123) GetRoot(ctx context.Context) (model.Obj, error) {
	return &yaegi_storage.ObjectHelper{
		ID:       d.RootFolderID,
		Name:     "root",
		Modified: d.Modified,
		IsFolder: true,
...
func (d *Pan123) List(ctx context.Context, dir model.Obj, args model.ListArgs) ([]model.Obj, error) {
	...
	newFile := make([]model.Obj, len(files))
	for i := 0; i < len(files); i++ {
		file := &files[i]
		newFile[i] = &yaegi_storage.ObjectHelper{
			IValue:   file,
			ID:       file.GetID(),
			Path:     file.GetPath(),
			Name:     file.GetName(),
...
var _ yaegi_storage.DriverPlugin = (*Pan123)(nil)
